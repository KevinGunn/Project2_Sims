...
# IPWE
IPWE <- function( Y, A, X, prop_score, eta){
...
opt.eta <- function(eta, Y, Z, A, X, prop_score, M, lambda){
  ...
  VY <- IPWE(Y, A, X, prop_score, eta)
  VZ <- IPWE(Z, A, X, prop_score, eta)
...
QP_IRL <- function(Y, Z, A, X, prop_score, k.num, eps, lambda, eta0){
  ...
  while(k <= k.num){
    ...
    plot(L_mu_mat[-1,-1],col="blue", pch=19, main="Value Function Estimation",
         xlab="Reward", ylab = "Proportion of Deaths in Hospital", cex=1.2, ylim = c(0,0.5), xlim = c(0,1))
    ...
    for(j in 1:nrow(eta_start)){
      fit<-try(optim(par=eta_start[j,],fn=opt.eta,method="Nelder-Mead",Y=Y,Z=Z,A=A,X=X,prop_score=prop_score,
...
# Covariates of interest.
# lambda = 0.015 sets the number of variables to about 10.
...
lasso_prop <- glmnet(x=xvars,y=sepdat_iv_dose_b1$iv_dose, family="binomial",alpha=1, lambda=0.035)
prop_fit <- predict(lasso_prop, type = "response", newx = xvars)
...
eta0_guess <- eta0[5,]
#eta0_guess <- eta0[3,]
#eta0_guess <- rep(1,npar)
#eta0_guess <- rep(-1,npar)
#eta0_1 <- rep(-0.5,npar)
#eta0_2 <- rep(0.1,npar)
#eta0_3 <- rep(c(0,0.3),npar/2)


# QP-IRL application.
AL_sep <- QP_IRL( Y= sepdat_iv_dose_b1$died_in_hosp, Z=sepdat_iv_dose_b1$mortality_90d , A=sepdat_iv_dose_b1$iv_dose, 
                  X = xvars_final , prop_score = prop_fit , k.num=50, eps = 0.00001, 
                  lambda=0, eta0 = eta0_guess )


capture.output(AL_sep,file = "AL_sep_eta_all1.txt")
...
# save output
capture.output(AL_sep,file = "AL_sep_eta01.txt")

# QP-IRL application.
AL_sep <- QP_IRL( Y= sepdat_iv_dose_b1$died_in_hosp, Z=sepdat_iv_dose_b1$mortality_90d , A=sepdat_iv_dose_b1$iv_dose, 
                  X = xvars_final , prop_score = prop_fit , k.num=50, eps = 0.00001, 
                  lambda=0, eta0 = eta0_3 )

# save output
capture.output(AL_sep,file = "AL_sep_eta03.txt")
...
# save output
capture.output(AL_sep,file = "AL_sep_eta02.txt")
